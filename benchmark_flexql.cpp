...
static const long long DEFAULT_INSERT_ROWS = 1000000LL; // 100k rows by default for insertion benchmark
...
static bool run_insert_benchmark(FlexQL *db, long long target_rows) {
    ...
    while (inserted < target_rows) {
        std::string sql;
        sql.reserve(INSERT_BATCH_SIZE * 80);
        sql.append("INSERT INTO BIG_USERS VALUES ");
        ...
        while (in_batch < INSERT_BATCH_SIZE && inserted < target_rows) {
            ...
            sql.append(std::to_string(id));
            sql.append("@mail.com', ");
            sql.append(std::to_string(1000.0 + (id % 10000)));

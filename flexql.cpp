...
int flexql_open(const char *host, int port, FlexQL **outDb) {
    ...
    int opt = 1;
    setsockopt(db->sock, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt));
    int buf_size = 4 * 1024 * 1024; // 4MB
    setsockopt(db->sock, SOL_SOCKET, SO_SNDBUF, &buf_size, sizeof(buf_size));
    setsockopt(db->sock, SOL_SOCKET, SO_RCVBUF, &buf_size, sizeof(buf_size));
